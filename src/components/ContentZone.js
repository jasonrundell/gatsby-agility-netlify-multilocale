...
export default class ContentZone extends Component {
  renderModules = () => {
    ...
    const modulesForThisContentZone = this.props.pageContext.page.zones[
      contentZoneName
    ]
    ...
    modulesForThisContentZone.forEach(moduleItem => {
      ...
      const ModuleComponentToRender = this.props.modules[
        moduleItem.item.properties.definitionName
      ]
      const metaData = {
  ...
  render() {
    return <div className="content-zone">{this.renderModules()}</div>
  }
}
