  ...
  <StaticQuery
    query={graphql`
      query PostListingModuleQuery {
        ...
        allAgilitySitemapNode(filter: { contentID: { ne: null } }) {
          nodes {
            contentID
            path
          }
    ...
    render={queryData => {
      ...
      queryData.allAgilityContentPost.nodes.forEach(post => {
        const sitemapNodeForPost = queryData.allAgilitySitemapNode.nodes.find(
          sitemapNode => {
      ...
      this.props.posts.forEach(post => {
        posts.push(
          <div className="post" key={post.contentID}>
            {post.myFields.image && (
              <img
                src={post.myFields.image.url + '?w=860'}
                alt={post.myFields.image.label}
              />
            )}
            <h2>
            ...
            <p
              dangerouslySetInnerHTML={this.renderPostExcerpt(
                post.myFields.details
              )}
            ></p>
          </div>
        )
